---
name: agent-mailbox
description: CLI をまたぐエージェント間の役割分担・下請け通信を行うときに使う。scripts/agent_mailbox.py の register / send / deliver / close の手順、ACK 契約、CLI 別の --agent-cmd、role 交代の作法を含む。native subagent の代わりに別 CLI / 別モデルへ仕事を渡したいとき、mailbox の運用・トラブル対応をするときに使う。
---
